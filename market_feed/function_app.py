...
def _validate_token(request: func.HttpRequest) -> None:
...
def quote(request: func.HttpRequest) -> func.HttpResponse:
    ...
    if not symbol:
        return _json_response({"detail": "symbol is required"}, status_code=400)
    ...
    try:
        ...
        return _json_response({"detail": str(exc)}, status_code=503)

    if payload.get("status") == "error":
        return _json_response(
            {
                "detail": payload.get("message", "Market-data provider returned an error"),
